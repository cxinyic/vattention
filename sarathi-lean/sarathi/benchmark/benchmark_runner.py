...
class UpgradeState:
    """Shared state for coordinating decode serving during upgrade"""
    def __init__(self):
...
class BenchmarkRunner:
    ...
    def _add_requests(self) -> None:
        ...
        logger.info(f"Adding {len(self._requests)} requests to engine")
    ...
    def prepare_for_decode_upgrade(self) -> dict:
        ...
        # TODO(XY): change the hardcode required_blocks to a parameter
        required_blocks = self._config.upgrade_required_blocks
    ...
    def run(self) -> None:
        ...
        if self._config.upgrade_strategy == UpgradeStrategy.DECODE_UPGRADE:
            return self._run_with_decode()
        elif self._config.upgrade_strategy == UpgradeStrategy.PREFILL_UPGRADE:
            return self._run_with_prefill()
        elif self._config.upgrade_strategy == UpgradeStrategy.BASIC_UPGRADE:
            return self._run_without_serve()
        else:
            return self._run_normal()  # Just run normally for NO_UPGRADE

    def _run_with_decode(self) -> dict:
        """Run with decode serving during upgrade"""
        status = self._run_normal()

        if status == "UPGRADE_NEEDED":
            # Initialize upgrade state
            if self.upgrade_state is None:
                logger.error("Upgrade state not set. Cannot proceed with decode serving.")
                return {"status": "ERROR"}
    ...
    def _run_with_prefill(self) -> dict:
        """Run with decode serving during upgrade"""
        status = self._run_normal()
    ...
    def _run_without_serve(self) -> dict:
        """Run without decode serving during upgrade"""
        status = self._run_normal()
    ...
    def run_during_decode(self) -> dict:
        """Continue serving requests during decode phase"""
...
class BenchmarkRunnerLauncher:
    ...
    def run_with_upgrade(self):
        ...
        if not self._is_multi_replica:
            if self._config.upgrade_strategy == UpgradeStrategy.DECODE_UPGRADE:
                return self._run_with_decode_upgrade()
            elif self._config.upgrade_strategy == UpgradeStrategy.PREFILL_UPGRADE:
                return self._run_with_prefill_upgrade()
            elif self._config.upgrade_strategy == UpgradeStrategy.BASIC_UPGRADE:
                return self._run_without_serve_upgrade()
            else:
                return self._run_normal()  # Just run normally for NO_UPGRADE
        else:
    ...
    def _run_with_decode_upgrade(self):
        """Run single replica upgrade with decode serving"""
        ...
        if isinstance(result, dict) and result["status"] == "UPGRADE_NEEDED":
            ...
            result = self._runner.run_during_decode()
    ...
    def _run_with_prefill_upgrade(self):
        """Run single replica upgrade with decode serving"""
        upgrade_state = UpgradeState()
        new_runner = None
        saved_progress = None
    ...
    def _run_without_serve_upgrade(self):
        """Run single replica upgrade without decode serving"""
        result = self._runner.run()
